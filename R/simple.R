...
define_simple_wrapper <- function(data, id,
  ...
  if(!is.null(calib_weight)){
    if(!is.numeric(calib_weight))
      stop_("The weight after calibration (", arg$calib_weight, ") should be numeric.")
    if(anyNA(calib_weight[resp %in% TRUE]))
      stop_("The weight after calibration (", arg$calib_weight, ") should not contain any missing (NA) values for units used in the calibration process.")
  }
