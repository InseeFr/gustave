...
#'
#' @description \code{rescal} calculates linear regression residuals in an efficient way :
...
#'
#' @param y A numerical matrix of dependant variable(s). May be a Matrix::TsparseMatrix.
#' @param x A numerical matrix of independant variable(s). May be a Matrix::TsparseMatrix.
...
#' @param colinearity.check A boolean (\code{TRUE} or \code{FALSE}) or \code{NULL} indicating
#' whether to perform a check for colinearity or not (see Details).
#' @param precalc A list of precalculated results (see Details).
#'
#' @details In the context of the \code{gustave} package, linear regression residual
...
#'
#' @seealso \code{\link{Diagby}} \code{\link{varDT}}
#'
...
#'
#' @description \code{varDT} estimates the variance of the total in the case
...
#'
#' @param y A numerical matrix of the variable(s) whose variance of their total is to be estimated. variable(s). May be a Matrix::TsparseMatrix.
#' @param pik A numerical vector of first-order inclusion probabilties.
#' @param x An optional numerical matrix of balancing variable(s). May be a Matrix::TsparseMatrix.
#' @param strata An optional categorical vector (factor or character)
...
#' @param precalc A list of precalculated results (see Details).
#'
#' @details \code{varDT} is the workhorse of most variance estimation conducted with
...
#' @author Martin Chevalier (Insee)
#'
#' @seealso \code{\link{Diagby}} \code{\link{rescal}}
#'
#' @references Caron N., Deville J.-C., Sautory O. (1998), \emph{Estimation de précision
...
varDT <- function(y = NULL, pik, x = NULL, strata = NULL, w = NULL, colinearity.check = NULL, precalc = NULL){
  ...
  if(is.null(precalc)){
    ...
    inv <- as(if(det(u) != 0) solve(u) else MASS::ginv(as.matrix(u)),"TsparseMatrix")
