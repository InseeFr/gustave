...
#' TODO
total <- linwrap_factory(linfun = function(y, w){
  y[is.na(y)] <- 0
  total <- sum(y * w)
...
#' TODO
ratio <- linwrap_factory(linfun = function(num, denom, w){
...
#' TODO
mean <- linwrap_factory(linfun = function(y, w) environment(ratio)$linfun(
  num = y, denom = rep(1, length(y)), w = w
), allowfactors = TRUE)

#' TODO
diffratio <- linwrap_factory(linfun = function(num1, denom1, num2, denom2, w){
  na <- is.na(num1) | is.na(denom1) | is.na(num2) | is.na(denom2)
  num1[na] <- 0; denom1[na] <- 0; num2[na] <- 0; denom2[na] <- 0
  ratio1 <- environment(ratio)$linfun(num = num1, denom = denom1, w = w)
  ratio2 <- environment(ratio)$linfun(num = num2, denom = denom2, w = w)
