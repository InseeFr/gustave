...
Il est facile dans `qvar()` de tenir compte du fait que des unités ont été **détectées comme hors-champ par l'enquête** (présentes dans la base de sondage mais pas dans la suite des redressements).
...
Enfin, `qvar()` peut également tenir compte de l'**impact d'un calage sur marges** sur la précision des estimateurs.
...
Elle ne permet cependant pas de tenir compte des **plans de sondage complexes** (phases multiples, partage des poids, etc.), omniprésents dans les enquêtes ménages de l'Insee.

. . . 

\ 

Pour ces cas plus complexes, le *package* gustave propose une **boîte à outils de fonctions** permettant au ou à la méthodologue de **construire un *wrapper* de variance sur-mesure**.
...
- `y` : variables d'intérêtsur lesquelles faire porter l'estimation de variance ;
...
**Large éventail de méthodologies couvert** : méthodes d'échantillonage complexes (échantillon-maître des enquêtes ménages de l'Insee), partage des poids complexes (EU-Silc).
...
L'apport du *package* Matrix est tel qu'**une suppression de cette dépendance n'appraît néanmoins pas opportune**.

## Sources de complexité dans le développement

La finalité de gustave étend de **simplifier** le travail du ou de la méthodologue et l'utilisation par le ou la non-spécialiste, **il incorpore tout un ensemble de fonctionnalités** en pratique complexe à développer et à maintenir : 
...
- nombreux **tests méthodologiques réalisés par `qvar()`** : `qvar()` comportant de nombreux tests vérifiant la cohérence des paramètres saisis, tous ces tests doivent être **eux-mêmes testés systématiquement**.

## Principes de développement

\ 

- Recherche de **stabilité** : pas d'ajout de dépendances trop lourde.
